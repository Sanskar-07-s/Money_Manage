...
import { X, Save, Tag, FileText, CheckCircle2 } from 'lucide-react';
import { cn } from '../utils/cn';
import GlassBox from './GlassBox';
import { updateLocalTransaction, getLocalCategories } from '../utils/storage';
import { updateTransaction, logAIEvent } from '../services/api';

export default function EditTransactionModal({ transaction, isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen && transaction) {
      setFormData({
        ...
        account: transaction.account || 'UPI',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        await logAIEvent('transaction_updated', transaction, updates);
      } catch (err) {
        console.warn("Cloud sync/AI log failed for edit, updating locally only.");
      }

      // 2. Update Local Storage (Rebuilds truth source balances)
      updateLocalTransaction(transaction.id, updates);
        ...
        <GlassBox className="p-8 shadow-2xl border-white relative overflow-hidden">
          {success ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h2 className="text-2xl font-display font-bold text-slate-800">Transaction Updated</h2>
              <p className="text-slate-500 mt-2 font-medium">Balance ledger recalculated.</p>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-2xl font-display font-bold text-slate-800 tracking-tight">Edit Transaction</h2>
                  <p className="text-amber-500 font-black text-[10px] mt-1 uppercase tracking-widest bg-amber-50 inline-block px-2 py-1 rounded-md">Modifications tracked</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {/* Note Field */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Meta Description (Note)</label>
                    <div className="relative">
